...
func (d *Database) GetComment(ctx context.Context, uuid string) (comment.Comment, error) {
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("error fetcing the comment by uuid: %w", err)
	}
...
func (d *Database) UpdateComment(ctx context.Context, id string, cmt comment.Comment) (comment.Comment, error) {
	cmttRow := CommentRow{
		ID:     cmt.ID,
		Slug:   sql.NullString{String: cmt.Slug, Valid: true},
		Author: sql.NullString{String: cmt.Body, Valid: true},
		Body:   sql.NullString{String: cmt.Author, Valid: true},
	}
	...
	return convertCommentRowToComment(cmttRow), nil
}
